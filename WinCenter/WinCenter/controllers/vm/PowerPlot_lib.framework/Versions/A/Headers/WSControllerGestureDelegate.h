...
/// This protocol is implemented by a delegate for control gestures of
/// a @p WSPlotController. The most common use is a single tap
...
/** @brief The range for a hit-test on data points.
    ...
    location and the resulting data point. The controller's @p
...
/** @brief Return closest datum to current tap location.
    ...
    This method is called when the controller's @p hitResponseMethod
    is set to @p kHitResponseDatum.
 
    If no data point is found within @p singleTapRange around the tap
    location, -1 is returned. If several data points are found, the
    one closest to the tap location is returned. The definition of
    'close' is based on the @p hitTestMethod property of the
    controller.
 
    @param controller The controller who manages the gesture
           recognizer.
...
/** Return actual coordinates of the tap in the controller's view.
    ...
    This method is called when the controller's @p hitResponseMethod
    is set to @p kHitResponseLocationBounds.
...
/** Return actual coordinates of the tap in the controller's view.
    ...
    This method is called when the controller's @p hitResponseMethod
    is set to @p kHitResponseLocationData.
