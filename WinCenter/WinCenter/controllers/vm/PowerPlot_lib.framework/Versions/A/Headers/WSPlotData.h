...
typedef NS_ENUM(NSInteger, WSLineStyle) {
...
typedef NS_ENUM(NSInteger, WSInterpolationStyle) {
...
/// (with a symbol the user can choose) and can be connected by a line
/// with a color, width and dashing the user can choose. Furthermore,
/// the data can be filled down to the coordinate axis; the filling
/// color and gradient can also be chosen by the user.
///
/// Since version 1.2.2 shadows for the line and symbols are supported
/// as well as different connection styles: straight lines (the
/// default) and Bezier curves which smoothly interpolate between the
/// available data points.
///
/// @note This class does not take the error correlation slot of @p
...
@property (strong, nonatomic) WSDataPointProperties *propDefault;
...
/** @brief Set the colors of a fillGradient with two colors.
    ...
    This method is a shortcut for using custom gradients. It allows to
    choose the gradient running from one color to another. If still
    more functionality is required, the user has to directly use
    setFillGradient with a custom gradient.
