...
/// can be filled with a color and optionally have a line around them.
/// If a data point has no uncertainty or if it is zero in one
/// direction, no symbol is drawn for that point. If a non-zero value
/// for @p errorCorr is specified, the disc for that point will be
/// tilted; the tilt corresponds to 45 degrees * @p errorCorr - thus,
/// meaningful values for @p errorCorr are from the interval @p [-1,
/// 1].
@interface WSPlotDisc : WSPlot <WSDatumCustomization>

/** Disc customization. */
@property (strong, nonatomic) WSDiscProperties *propDefault;
