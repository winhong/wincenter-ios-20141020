...
/// provided by the @p WSGraphConnections class. The methods cover the
/// minimum access methods any class that can handle this type of data
/// is required to implement. The @p WSGraph class implements all
/// methods in this class and thus a custom implementation is rarely
/// needed.
///
/// Although several methods are marked optional in this protocol,
/// failure to implement them may lead to some features of nodes not
/// working properly. In principle the required method is sufficient
/// to reproduce the missing functionality, the current implementation
/// of the @p WSNode category of @p WSDatum needs the optional methods
/// as defined here to provide all functionality.
...
/** Return the index of a given node in the graph.
    ...
    @param node The node object of class @p WSNode.
...
/** @brief Return a set of all connections between two nodes.
    ...
    between two nodes (or @p nil if there is none). This method can be
    ...
    @return A WSGraphConnections object containing all connecting
            links.
 */
