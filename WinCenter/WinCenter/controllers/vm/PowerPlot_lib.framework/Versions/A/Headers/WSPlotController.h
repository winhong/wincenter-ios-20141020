...
typedef NS_ENUM(NSInteger, WSTapHitTesting) {
...
typedef NS_ENUM(NSInteger, WSTapHitResponse) {
    kHitResponseNone,           ///< Do not call the delegate.
    kHitResponseDatum,          ///< Identify the datum index (based on @p hitTestMethod), call the delegate.
...
/// @brief Plot controller object handling one data set and one chart
/// component.
///
...
/// typically a subclass of @p WSPlot), the two coordinate axis (class
/// @p WSCoordinate), two axis locations (used by some, but not all
/// plots) and optionally an instance of @p WSData. In the latter case
/// it must implement the WSDataDelegate formal protocol that defines
/// methods for accessing the data.
///
/// This controller also implements the @p WSCoordinateDelegate formal
/// protocol that provides methods for conversion of screen/bounds and
/// data coordinates.
///
/// The instances of @p coordX and @p coordY handled by this
/// controller may be shared with other controllers. This can be used
/// to give the chart a unified coordinate and axis system if
/// needed. Similarly, @p axisLocationX and @p axisLocationY can be
/// shared among several plot controllers, but it remains the user's
/// responsibility to make sure that no inconsistencies arise,
/// particularly with the delegate of these classes!
///
/// It is, of course, possible to also use other view objects (like
/// UILabel instances etc.) in this controller. That is a mechanism to
/// add more customization to a plot.
///
...
/// work, the @p zoomRange[X|Y]D and @p scrollRange[X|Y]D properties
/// must be set up appropriately. For configuring a response to tap
/// gestures, set the delegate property of this class.
///
/// @note An instance of @p WSPlotController is <strong>not</strong> a
///       user interface view controller, but rather an abstraction to
///       combine data model objects and view objects. Although its
///       view is accessed only indirectly via @p [instVar view], from
///       the users' point of view it is a view which is internally
///       split up according to the MVC paradigm.  This is the reason
///       why @p WSPlotController inherits directly from @p WSObject
///       rather from a @p UIController or a @p UIView.
@interface WSPlotController : NSObject <UIGestureRecognizerDelegate,
     WSAxisLocationDelegate, WSCoordinateDelegate, WSDataDelegate>

@property (strong, nonatomic) WSPlot *view;                                       ///< View handled by this controller.
@property (strong, nonatomic) WSAxisLocation *axisLocationX;                      ///< Handles X-axis positioning.
@property (strong, nonatomic) WSAxisLocation *axisLocationY;                      ///< Handles Y-axis positioning.
@property (strong, nonatomic) WSCoordinate *coordX;                               ///< Abscissa axis.
@property (strong, nonatomic) WSCoordinate *coordY;                               ///< Ordinate axis.
...
@property (weak, nonatomic) id<WSControllerGestureDelegate> delegate;             ///< Delegate for gestures.

/** @brief Configure zooming factors for pinch gestures.
 
    This method sets the @p rangePinchXD, @p rangePinchYD, @p
    scrollRangeXD and @p scrollRangeYD properties based on the current
    coordinates. It is an alternative to computing and setting these
    ranges manually.
...
/** @brief Activate data bindings for this specific plot.
    ...
    subclass of @p WSPlot is changed and this flag is turned on, then
    the graph will automatically be redrawn.
...
/** @brief Activate alerting for this specific plot.
    ...
    @p WSPlot that support customizable data, like @p WSPlotBar or @p
    WSPlotGraph etc.  The style property in the view must be set to @p
    kCustomStyleIndividual.
 
    In order to configure automatic alerting, the bindings property
    has to be set to TRUE. For manual alerting call @p
    updatedAlertedData. In either case, the @p standardProperties and
    @p alertedProperties must be set to the correct values.
 
    The @p standardProperties and the @p alertedProperties properties
    are the values of the @p customDatum of a @p WSDatum that is not
    alerted and alerted, respectively.
 */
...
/** Perform an update of the @p dataD data set based on the alerting
    configuration.
 */
