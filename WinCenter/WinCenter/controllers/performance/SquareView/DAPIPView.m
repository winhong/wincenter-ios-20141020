...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    NSDictionary *d = [NSDictionary dictionaryWithObject:[NSValue valueWithCGPoint:super.center] forKey:BNRChangedNotfication];
...
- (void)moveToCornerUnit:(CGPoint)unit animated:(BOOL)animated
{
   ...
   // NSLog(@"%f",unit.y);
    ...
    cornerPoint = CGPointMake((xCenter) + ((xWidth/2.0f+44.5f) * unit.x),
                              (yCenter) + ((yHeight/2.0f+44.5f) * unit.y));
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(xCenter-50, yCenter-50, 100, 100)];
    view.backgroundColor = [UIColor clearColor];
    view.hidden = YES;
    
    [self.superview addSubview:view];
    
    if(CGRectContainsPoint(view.frame,super.center)){
         super.center = CGPointMake(xCenter, yCenter);
       
    }else{
        
        if(isnan(cornerPoint.x) || isnan(cornerPoint.y)){
            super.center = CGPointMake(xCenter, yCenter);
        }else{
    ...
    NSMutableDictionary *d = [NSMutableDictionary dictionaryWithObject:[NSValue valueWithCGPoint:super.center] forKey:BNRChangedNotfication];
    ...
    [UIView setAnimationDidStopSelector:@selector(animationComplate)];
    [UIView commitAnimations];
